...
- **Settings pop-up (`components/SettingsModal.tsx`):** Lets the admin pick, per browser (localStorage, no Firestore), which model each pipeline stage uses, with pros/cons and cost notes shown inline. Site defaults are unchanged — the pop-up only overrides them when the admin explicitly picks something else.
...
- **Pipeline:** `useTranscriberPipeline.ts` orchestrates: read the stored model choices → validate file → upload (with real progress via XHR) → `POST /api/transcriber/transcribe` (with the chosen transcription model) → chunk the resulting segments with overlap → `POST /api/transcriber/correct` once per chunk (with the chosen correction model) → stitch chunk results → build the final `.txt`.
