...
- **Requirements panel (`components/RequirementsPanel.tsx`):** Rendered at the top of the tool (below the header, above the upload panel). Calls `GET /api/transcriber/status` with a freshly-forced ID token (`user.reload()` then `getIdToken(true)`) to explain, before a run, exactly which of `requireAdminUser`'s checks would fail and why — most commonly an unverified email, which otherwise only surfaces as a generic 401 mid-run. Includes a "Resend verification email" action (`sendEmailVerification` from the Firebase client SDK) and a "recheck" action. This panel is explanatory only, same as the rest of the client-side UI gating — the API routes remain the actual enforcement point.
- **Status route (`app/api/transcriber/status/route.ts`):** Read-only, GET, no transcript/model calls. Verifies the bearer token (via `verifyFirebaseIdToken`, not `requireAdminUser`, so it can report granular pass/fail instead of throwing) and returns whether the token is present/valid, whether its email matches `ADMIN_EMAIL`, and whether `email_verified` is true. Server-side key presence (`GPT_API_KEY`/`GEMINI_API_KEY` set or not — never the values) is only included once the caller already clears both the email-match and email-verified bar, so this route can't be used to probe deployment config as a non-admin or unverified caller.
- **Settings pop-up (`components/SettingsModal.tsx`):** Lets the admin pick, per browser (localStorage, no Firestore), which model each pipeline stage uses, with pros/cons and cost notes shown inline. Site defaults are unchanged — the pop-up only overrides them when the admin explicitly picks something else. It also documents (read-only, not editable) the fixed correction-prompt rules, chunk/overlap window, and temperature, with a worked before/after example, so the free-text `Speakers`/`Context notes` fields on the upload panel are easier to use well.
  - **Transcription model:** `app/lib/transcribeModels.ts` lists only OpenAI models that return segment-level timestamps (`gpt-4o-transcribe-diarize`, `whisper-1`) — the pipeline's chunk/stitch logic depends on per-segment start/end times, which `gpt-4o-transcribe`/`gpt-4o-mini-transcribe` don't provide (`json`/`text` response formats only), so those aren't offered as options.
  - **Correction model:** reuses the app-wide `AVAILABLE_GEMINI_MODELS` catalog from `app/lib/aiModels.ts` (same list/UI pattern as the Movie/TV Show Tracker's settings page) since any Gemini text model works interchangeably for the JSON correction task.
- **Pipeline:** `useTranscriberPipeline.ts` orchestrates: read the stored model choices → validate file → upload (with real progress via XHR) → `POST /api/transcriber/transcribe` (with the chosen transcription model) → either chunk the resulting segments with overlap and `POST /api/transcriber/correct` once per chunk (with the chosen correction model) then stitch chunk results, or — when **skip cleanup pass** is enabled — skip straight to building the transcript from the raw segments, with `lib/buildManualCleanupPrompt.ts`'s prose prompt prepended → build the final `.txt`.
- **Transcribe route (`app/api/transcriber/transcribe/route.ts`):** Re-validates the 25 MB OpenAI upload limit server-side. Resolves the requested model via `resolveTranscribeModelId` (falls back to the default, `gpt-4o-transcribe-diarize`, if missing/invalid) and branches the OpenAI request format by capability: diarization-capable models use `response_format: diarized_json` + `chunking_strategy: auto`; others use `verbose_json` + segment-level `timestamp_granularities`, same as the fallback path. OpenAI labels distinct speakers sequentially ("A", "B", …) since no voice-sample references are collected in this UI; `lib/mapSpeakerLabels.ts` maps those onto the user-provided speaker names in first-appearance order (extra speakers beyond the provided names become `Unknown`). On any failure of the selected model/endpoint (and the selected model isn't already the fallback), retries once with `whisper-1` + `verbose_json`.
- **Correct route (`app/api/transcriber/correct/route.ts`):** Runs one Gemini call per chunk (model resolved via `resolveGeminiModelId`, default `gemini-2.5-flash`, low temperature) with a strict-JSON-out prompt (`lib/buildCorrectionPrompt.ts`) that forbids summarizing/rewriting and asks only for `{index, speaker, text}` per segment — timestamps are never trusted from the model and are always taken from the original segment. Not called at all when **skip cleanup pass** is enabled.
- **Skip cleanup pass:** an upload-panel toggle (`UploadPanel.tsx`) that bypasses the Gemini correction pass entirely (and disables **strict mode**, which has nothing to fail against). `useTranscriberPipeline.ts` builds the raw transcript directly from the transcribe route's segments and prepends `buildManualCleanupPrompt()`'s output — the same rules as `buildCorrectionPrompt.ts`, in prose instead of strict-JSON-out, written for a human to paste (prompt + transcript together) into a general-purpose browser AI chat instead of this tool's built-in correction pass.
