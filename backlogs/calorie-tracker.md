...
- [p] **#32 — Chart.js 3.9.1 is two majors behind**
  ...
  > pushed — upgraded CDN from 3.9.1 to 4.5.0 (chart.umd.min.js); added explicit type:'linear' to y2 scale per v4 migration; tests: 575 pass; commit: _pending_
...
- [p] **#42 — No PWA / Add to Home Screen support**
  ...
  > pushed — manifest.json + sw.js with app shell precache; SVG+PNG icons (192/512); cache-first for local assets, stale-while-revalidate for CDN, network-only for Firebase; tests: 575 pass; commit: _pending_
...
- [p] **#43 — No CSV bulk-import for saved foods**
  ...
  > pushed — importSavedFoodsCsv in exporters.js; parses RFC 4180 CSV with header mapping; validates name column; writes each food to Firestore; wired via hidden file input + button in Settings; tests: 575 pass; commit: _pending_
