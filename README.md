...
### CIFI - Research Estimator

_“Estimate CIFI research payout timing with logged rate history and interactive forecast charts.”_
...
_"Paste a ChatGPT-converted recipe as strict JSON, then stage prep groups, execute with named results, scale, and save — a phone-first kitchen workflow."_

This React tool lives under **app/tools/recipe-standardizer/**. The site makes no AI API calls: it provides a copyable ChatGPT conversion prompt, the user converts a recipe to strict JSON in ChatGPT manually, and pastes the JSON back. The importer validates strictly (exact-path errors, plus structural chronology checks over the workflow references), then renders the recipe as accordion sections (Overview / Ingredients / Prep / Execution / Scaling / JSON — only Overview open by default) with a one-line sticky jump bar, gram-first quantities with secondary equivalents, and non-destructive scaling (servings / multiplier / target weight / portions). Recipes are organized around named intermediates: prep groups (named staged inputs with a destination, a schedule — at the start, during a named wait, after a section, or just in time — a holding note, and an exact first-use step) and named step results ("crust mixture") that later steps consume, so execution steps read "transfer the crust mixture" instead of re-listing ingredients (the underlying quantities stay one collapsed "Verify contents" disclosure away). The Overview timeline separates active work from passive waits, nests work that happens during a wait, and keeps alternative timing paths in a collapsed disclosure. Older saved recipes (schema v1) load silently through a compatibility path and render through the same views with conservatively derived groups. The Ingredients panel groups rows by prep group with modes for Shopping / Pantry and Grocery Category; renaming an ingredient updates all structured displays and opens a review of affected step text so instructions never silently contradict the substitution. Recipes save per-user to `artifacts/recipe-standardizer/users/{uid}/recipes`, and edits to a saved recipe always prompt update / save-as-new / cancel. Ingredients carry a `nutritionLink` hook that name-matches against the Calorie Tracker's saved food items for future nutrition integration.
...
This React tool lives under **app/tools/conflict-tracker/** and follows the same Firebase Auth + Firestore architecture as Trip Planner and Date Night.

Key features:
- **Two-person tracker:** Conflicts are grouped under a shared tracker representing a pair of people. The creator defaults to Person A; Person B claims their role by joining via email invite or directly in the app.
- **Independent reflections:** Each person fills out their reflection privately after things have cooled down. Reflections remain hidden from the partner until both sides submit.
- **Reveal on both submit:** Once both reflections are submitted, they appear side-by-side with an interpretation comparison ("what I thought they meant" vs. what they actually meant).
- **Shared section:** After both reflections are submitted, a shared section unlocks for mutual clarifications and commitments (editable by Person A or admin for MVP).
- **Per-person resolution:** Each person independently marks the conflict as resolved via checkboxes. Status is computed as open / partially_resolved / resolved.
- **Trend dashboard:** Aggregates tag frequency, repeated unmet needs, severity sparkline, resolution counts, active commitments, and repair highlights across all conflicts in the tracker.
- **Custom tags:** Default tag suggestions plus free-entry custom tags saved per tracker.
...
This React tool lives under **app/tools/transcriber/** with Edge API routes in **app/api/transcriber/**. Unlike every other Firebase-backed tool on this site, it is restricted to a single account (the site admin) rather than any signed-in user, and it has no Firestore persistence at all — nothing about a transcription run is saved once the browser tab closes (settings and speaker-profile data persist locally in that browser's localStorage/IndexedDB, never server-side).

Key features:
- **Upload:** accepts a single recording (`.m4a`, `.mp3`, `.wav`, `.aac`, `.ogg`, or `.webm`), validated client- and server-side against a provider-aware upload cap (25 MB for the OpenAI providers, or 95 MB when OpenAI's auto-optimize/chunking setting is on — see below; 95 MB for Gemini), with speaker names/voice notes managed in a Speaker Profiles panel (defaulting to Kait/James, with optional recorded or uploaded voice-reference clips) and optional free-text context notes.
- **Long/large OpenAI recordings:** `gpt-4o-transcribe-diarize` caps audio at 1500s and OpenAI caps a single upload at 25 MB. When a recording exceeds either (and the "auto-optimize & chunk" setting is on, default), the browser decodes it once, removes long silences, optionally speeds it up (default 1.2×, raising pitch slightly — configurable down to 1.0×), splits it into chunks under both caps, transcribes each chunk through the same route, and stitches the results back into one transcript with every timestamp remapped to the original recording's time — the original file is never itself uploaded to the server in this path.
- **Transcription providers:** an inline picker selects between OpenAI `gpt-4o-transcribe-diarize` (`response_format: diarized_json`, speaker labels mapped onto the provided names, optional known-speaker reference clips), OpenAI `whisper-1` (timestamps only, no speaker labels), and direct Gemini transcription (client-orchestrated Files API upload plus one or more windowed `generateContent` calls for long recordings, speaker identity inferred from names/notes/context). An optional auto-fallback setting hops to a backup provider on failure; otherwise a diagnostics-and-retry panel appears.
- **Cleanup pass:** the raw transcript is split into overlapping ~15-minute windows and sent chunk-by-chunk to Gemini with a strict JSON-only, preservation-first prompt that fixes obvious speaker misattributions and transcription/punctuation errors without summarizing, rewriting, or softening the wording; uncertain speakers are left `Unknown` rather than guessed. Conservative hallucinated-filler suppression and same-speaker turn-merging run around this pass, and an optional argument-relevance tag can be requested in the same call. Chunk results are stitched back together by keeping only each window's non-overlapping "core" region, which prevents duplicated lines from the overlap by construction.
- **Correction failure handling:** if a chunk's correction call fails, its uncorrected segments are kept (never dropped) and the failure is counted; the final screen shows a clear "Completed with warnings: N of M correction chunks failed and were left uncorrected" banner when that happens. An optional strict-correction toggle aborts the whole cleanup pass on the first correction failure instead of falling back silently.
- **Output:** every successful run produces both a raw and a cleaned `[HH:MM:SS] Speaker: text` `.txt` transcript (Download/Copy for each, raw collapsed by default), plus a step-by-step progress view with elapsed time.
- **Access control:** gated to the exact admin email both in the UI (so other signed-in users never see the tool) and independently in every API route via a hand-rolled Firebase ID token verifier (`app/lib/verifyFirebaseAuth.ts`) — see `SECURITY.md` for why this doesn't use `firebase-admin`. The sign-in screen is login-only; account creation isn't offered since only the admin account can ever use the tool.

### Date Night Roulette (Firebase-backed)

_"Spin to pick a random date idea and modifier, log how it went."_

This React tool lives under **app/tools/date-night/** and mirrors the Trip Planner architecture with Firebase Auth + Firestore listeners, plus wheel-specific pure logic modules.

Key features:
- **SVG wheel sequence:** rarity + item wheels for dates and modifiers with weighted slices, pointer landing math, and spin easing.
...
### Trip Planner (Firebase-backed)

_“Plan collaborative itineraries with timelines, saved ideas, and a future map view.”_

This React tool lives under **app/tools/trip-planner/** and now mirrors the Trip Cost Firebase integration end-to-end:

- **Authentication:** Reuses the existing Firebase Auth configuration so users sign in with the same credentials they already use for Trip Cost.
- **Realtime Firestore:** Planner metadata, scheduled events, activity ideas, and audit history are stored under `artifacts/trip-planner/**`. Helper modules (`lib/firebase.ts`, `lib/db.ts`, `lib/image.ts`) encapsulate Firebase initialization, typed collection builders, and client-side uploads.
- **State management:** `PlanContext.tsx` subscribes to the planner, events, ideas, and changelog collections, exposing CRUD helpers and utilities for uploads, audit logging, and computed day schedules.
- **Timeline UI:** A drag-to-move, resize-to-adjust grid that renders itinerary items across days with customizable increments.
- **Idea Library:** Side panel for searching curated activities and injecting them into the schedule.
- **Settings & Map Preview:** Controls for timeline cadence, timezone, and a placeholder map area that will eventually stream Leaflet + OSM tiles.
